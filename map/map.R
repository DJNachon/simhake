...
breaks <- quantile(L50$L50, probs = seq(0, 1, length.out = 6))
...
ggplot() +
   ...
   geom_sf(data = combined_areas[combined_areas$source == "GFCM",], 
           fill = "#8ea8c3", color = fill_land2, size = 0.1, alpha = 0) +
   geom_jitter(data = L50, aes(x = Lon, y = Lat, size = L50), 
...
ggplot() +
   geom_tile(data = sst_df, aes(x = long, y = lat, fill = SST), alpha = 0.9) + 
   scale_fill_gradientn(colors = palette) +
   geom_sf(data = world, fill = fill_land, color = fill_land2) +
   geom_sf(data = combined_areas[combined_areas$source == "ICES",], 
           fill = "#8ea8c3", color = fill_land2, size = 0.1, alpha = 0) +
   geom_sf(data = combined_areas[combined_areas$source == "GFCM",], 
           fill = "#8ea8c3", color = fill_land2, size = 0.1, alpha = 0) +
   geom_jitter(data = L50, aes(x = Lon, y = Lat, size = L50, color = Stock),
               alpha = ifelse(L50$Stock == "Mediterranean Sea", 0.6, 0.15), 
               width = 0.8, height = 0.7) +
   scale_size_continuous(range = c(1, 5), breaks = breaks, labels = breaks) +
   ...
   xlab(" ") + 
   ylab(" ") + 
   labs(fill = 'SST (ºC)', size = expression(L[50] ~ "(cm)"), color = "Stock", alpha = "Stock Alpha") +
   guides(fill = guide_colorbar(order = 1, title.position = "top", label.position = "bottom"),
   ...
   theme(legend.spacing.x = unit(0.5, 'cm'), legend.spacing.y = unit(0.5, 'cm')) 



ggsave("./Figure/MAP_L50-T.jpeg", dpi = 300, width = 10, height = 6)
...
main_plot <- ggplot() +
   geom_tile(data = sst_df, aes(x = long, y = lat, fill = SST), alpha = 0.9) + 
   scale_fill_gradientn(colors = palette, guide = "none") + 
   geom_sf(data = world, fill = fill_land, color = fill_land2) +
   geom_sf(data = combined_areas[combined_areas$source == "ICES",], 
           fill = "#8ea8c3", color = fill_land2, size = 0.1, alpha = 0) +
   geom_sf(data = combined_areas[combined_areas$source == "GFCM",], 
   ...
   labs(size = expression(L[50] ~ "(cm)"), color = "Stock") + 
   theme(legend.position = "bottom") + 
   theme(legend.box = "horizontal", legend.box.just = "bottom") + 
   theme(legend.spacing.x = unit(0.5, 'cm'), legend.spacing.y = unit(0.5, 'cm'))  
...
legend_plot <- ggplot() +
   ...
   guides(fill = guide_colorbar(title.position = "top", label.position = "right"))

sst_legend <- get_legend(legend_plot)

final_plot <- plot_grid(main_plot, sst_legend, ncol = 2, rel_widths = c(4, 1))

print(final_plot)

ggsave("./Figure/MAP_L50-T_2.jpeg", dpi = 300, width = 10, height = 6)
